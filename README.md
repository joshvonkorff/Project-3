...
Next, the number of words in each set of speeches can range from 4,000 to 140,000, so the number of appearances is multiplied by (140,000 / length) to get the number that would appear in 140,000 words. Then, the log is taken.  This number is added to the weight to get the final weight value. This takes into account that we care more about words that appear many times as opposed to only once.  
